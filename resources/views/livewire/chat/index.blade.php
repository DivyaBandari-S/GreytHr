...
@if ($conversations)
     ...
     <li id="conversation-{{$conversation->id}}" wire:key="{{$conversation->id}}"
                        ...
                        <img style="border-radius: 50%; margin-left: auto; margin-right: auto; display: block; height: 40px; width: 40px; margin-top: 5px;"
                            src="{{ asset('storage/' . $conversation->getReceiver()->image) }}" class="card-img-top" alt="...">
                            ...
                            <a href="{{route('chat',$conversation->id)}}"
                                ...
                                <h6 class="truncate font-medium tracking-wider text-gray-900"
                                    ...
                                    <small class="text-gray-700"
                                        ...
                                        @endif
                                        @php
    $lastMessage = $conversation->messages ? $conversation->messages->last() : null;
@endphp
                                     <span>   <p class="grow truncate text-sm font-[100]"
                                            style="color: #555555;font-size:10px">  {{ Str::limit($lastMessage ? $lastMessage->body : '', 15) }}</p>
                             {{-- unread count --}}
                             @if ($conversation->unreadMessagesCount()>0)
                             <span  style="font-weight: bold; padding: 1px 4px; font-size: 0.75rem; border-radius: 12px; background-color: #007bff; color: #ffffff;">
...
.sent .message-time {
    color: white; /* Set the color of the sent message time to white */
}
