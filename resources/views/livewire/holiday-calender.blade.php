<div>
    ...
    <div class="container">
        <div class="row m-0 p-0">
            ...
            <div class="col-md-12 text-right d-flex justify-content-end">
                <select id="yearSelect" wire:change="selectYear($event.target.value)" class="dropdownPlaceholder">
                    <option class="optionDropdown" value="{{ $nextYear }}"
                        {{ $selectedYear == $nextYear ? 'selected' : '' }}>
                        {{ $nextYear }}
                    </option>
                    <option class="optionDropdown" value="{{ $initialSelectedYear }}"
                        {{ $selectedYear == $initialSelectedYear ? 'selected' : '' }}>
                        {{ $initialSelectedYear }}
                    </option>
                    @foreach ($previousYears as $year)
                        <option class="optionDropdown" value="{{ $year }}"
    ...
    <div class="hol-container" id="calendar{{ $selectedYear }}">
        <div class="row m-0">
            ...
            @foreach (range(1, 12) as $monthIndex)
                ...
                @if ($hasHolidays)
                    <!-- Only render months if there's any holiday data -->
                    <div class="col-md-3">
                                    ...
                                    @foreach ($entriesForMonth as $entry)
                                        <div class="fest grid-container">
                                            <div class="grid-item date-container">
                                                <h5 class="p-0 m-0">
                                                    {{ \Carbon\Carbon::parse($entry->date)->format('d') }}</h5>
                                                <p class="mb-0" style="font-size: 10px;">
                                                    {{ substr($entry->day, 0, 3) }}</p>
                                            </div>
            ...
            @if (!$hasHolidays)
                <div class="bg-white rounded border p-3 d-flex flex-column align-items-center"
                    style="margin: 50px auto; width:80%;">
                    <p class="noDataAvailable">No Data Available</p>
                    <p class="normalTextValue">There is no data available for the selected year. Please check again
                        later.</p>
                </div>
            @endif
        </div>
