<div class="container vh-100 d-flex align-items-center justify-content-center">
    <div class="col-md-5"> <!-- Increased width for better fit -->
        ...
        <div class="alert alert-success d-flex justify-content-between align-items-center">
            <span class="w-100 text-center">{{ session('success') }} <a href="{{ route('emplogin') }}" class="btn btn-link">Login</a></span>
        </div>
        @endif

        @if (session()->has('error'))
        <div class="alert alert-danger d-flex justify-content-between align-items-center">
            <span class="w-100 text-center" style="white-space: normal;">{{ session('error') }} <a href="{{ route('emplogin') }}" class="btn btn-link">Login</a></span> <!-- Centered the error message -->
        </div>
        @endif
