    ...
    overflow-x: auto;">
            <table>
                <tr class="first-header-row" style="background-color:#ebf5ff;border-bottom: 1px solid #cbd5e1;">
                    ...
                    <th></th>
                    <th></th>
                    <th style="font-weight:normal;font-size:12px;padding-top:16px;color:#778899;">Session&nbsp;1<i class="fa fa-caret-<?php echo e($this->moveCaretLeftSession1 ? 'left' : 'right'); ?>" style="cursor:pointer;" wire:click="toggleCaretDirectionForSession1"></i></th>
                    <!--[if BLOCK]><![endif]--><?php if($this->moveCaretLeftSession1==true): ?>
                    <th></th>
                    <th></th>
                    <th></th>
                    <th></th>
                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                    <th style="font-weight:normal;font-size:12px;padding-top:16px;color:#778899;">Session&nbsp;2<i class="fa fa-caret-<?php echo e($this->moveCaretLeftSession2 ? 'left' : 'right'); ?>" style="cursor:pointer;" wire:click="toggleCaretDirectionForSession2"></i></th>
                    <!--[if BLOCK]><![endif]--><?php if($this->moveCaretLeftSession2==true): ?>
                    <th></th>
                    <th></th>
                    <th></th>
                    <th></th>
                ...
                <!--[if BLOCK]><![endif]--><?php for($i = 1; $i <= $daysInMonth; $i++): ?> <?php $dateKey=str_pad($i, 2, '0' , STR_PAD_LEFT) . " " . $currentMonthRep . " " . $currentYear; $dateKeyForLookup=$currentYear . '-' .str_pad($currentMonth, 2, '0' , STR_PAD_LEFT) . '-' . str_pad($i, 2, '0' , STR_PAD_LEFT); if (in_array($dateKeyForLookup, $holiday)) { $holidayNote=true; } $timestamp=mktime(0, 0, 0, $currentMonth, $i, $currentYear); $dayName=date('D', $timestamp); $isWeekend=($dayName=='Sat' || $dayName=='Sun' ); $isPresent=$distinctDates->has($dateKeyForLookup);
                    $isDate=($dateKeyForLookup<$todaysDate); $swipeRecordExists=$swiperecord->contains(function ($record) use ($dateKeyForLookup) {
                        ...
                        <tr style="border-bottom: 1px solid #cbd5e1;background-color:<?php echo e($isDate? ( $isWeekend ? '#f8f8f8' : ($holidayNote ? '#f3faff' : ($isPresent|| $swipeRecordExists?  '#edfaed':'#fcf0f0'))) :'white'); ?>;">


                            <td class="date" style="font-weight:normal;font-size:12px;padding-top:16px;border-right:1px solid #cbd5e1;">
                                <p style="white-space:nowrap;">
                                    <?php echo e(str_pad($i, 2, '0', STR_PAD_LEFT)); ?>&nbsp;&nbsp;<?php echo e($currentMonthRep); ?>&nbsp;<?php echo e($currentYear); ?>(<?php echo e($dayName); ?>)
                            ...
                            <!--[if BLOCK]><![endif]--><?php if($distinctDates->has($dateKeyForLookup)): ?>
                            <?php
                            ...
                            <td style="font-weight:normal;font-size:12px;padding-top:16px;">
                                ...
                                <?php echo e(date('H:i', strtotime($record['first_in_time']))); ?>


                            </td>
                            <td style="font-weight:normal;font-size:12px;padding-top:16px;">
                                <!--[if BLOCK]><![endif]--><?php if(empty($record['last_out_time'])): ?>
                                <?php
                                $record['last_out_time']=$record['first_in_time'];
                                ?>
                                <?php echo e(date('H:i', strtotime($record['last_out_time']))); ?>
                            ...
                            <td style="font-weight:normal;font-size:12px;padding-top:16px;">
                                <!--[if BLOCK]><![endif]--><?php if($record['first_in_time']==$record['last_out_time']): ?>
                                00:00
                                <?php else: ?>
                                <?php echo e(str_pad($hours, 2, '0', STR_PAD_LEFT)); ?>:<?php echo e(str_pad($minutes, 2, '0', STR_PAD_LEFT)); ?>
                            ...
                            <td style="margin-left:10px; margin-top:20px; font-size:12px;color: <?php echo e($isDate ? ($isWeekend ? 'black' : ($holidayNote ? 'black' : ($distinctDates->has($dateKeyForLookup) ? 'black' : '#ff6666')) ):'black'); ?>">
                                <!--[if BLOCK]><![endif]--><?php if($isDate==true): ?>
                                <!--[if BLOCK]><![endif]--><?php if($isWeekend==true): ?>
                                O
                                <?php elseif($holidayNote==true): ?>
                                H
                                <?php elseif($distinctDates->has($dateKeyForLookup)): ?>
                                P
                            ...
                            <td>
                                <button type="button" style="font-size:12px;background-color:transparent;color:#24a7f8;border:none;text-decoration:underline;" wire:click="viewDetails('<?php echo e($dateKeyForLookup); ?>')">
                                    Info
                                </button>
                            </td>
                            ...
                            ?>
                            <td style="font-weight:normal;font-size:12px;padding-top:16px;"><?php echo e(date('H:i', strtotime($record['first_in_time']))); ?></td>
                            <!--[if BLOCK]><![endif]--><?php if($isLateBy10AM): ?>
                            <td style="font-weight:normal;font-size:12px;padding-top:16px;"><?php echo e($lateArrivalTime); ?></td>
                            <?php else: ?>
                            <td style="font-weight:normal;font-size:12px;padding-top:16px;">00:00</td>
                            <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                            <?php else: ?>
                            ...
                            $isEarlyBy07PM = $firstInTime->format('H:i') < '19:00' ; ?> <!--[if BLOCK]><![endif]--><?php if(empty($record['last_out_time'])): ?> <?php $record['last_out_time']=$record['first_in_time']; $firstInTime=\Carbon\Carbon::parse($record['last_out_time']); $lateArrivalTime=$firstInTime->diff(\Carbon\Carbon::parse('19:00'))->format('%H:%I');
                                $isEarlyBy07PM = $firstInTime->format('H:i') < '19:00' ; ?> <td style="font-weight:normal;font-size:12px;padding-top:16px;"><?php echo e(date('H:i', strtotime($record['last_out_time']))); ?></td>
                                    <?php else: ?>
                                    <td style="font-weight:normal;font-size:12px;padding-top:16px;"><?php echo e(date('H:i', strtotime($record['last_out_time']))); ?></td>
                                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                    <!--[if BLOCK]><![endif]--><?php if($isEarlyBy07PM): ?>
                                    <td style="font-weight:normal;font-size:12px;padding-top:16px;"><?php echo e($lateArrivalTime); ?></td>
                                    <?php else: ?>
                                    <td style="font-weight:normal;font-size:12px;padding-top:16px;">00:00</td>
                                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
...
</div><?php /**PATH C:\xampp\htdocs\GreytHr\resources\views/livewire/attendance-table.blade.php ENDPATH**/ ?>